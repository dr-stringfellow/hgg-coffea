...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        if self.year == 2016:
            filters.append("CSCTightHaloFilter")
        ...
        if self.partonmatch == 5:
            #events.GenPart = events.GenPart[(events.GenPart.status == 23) & (abs(events.GenPart.pdgId) < 6)]
            #events.GenPart = events.GenPart[(events.GenPart.status == 23)]
            events.GenPart = events.GenPart[(abs(events.GenPart.pdgId) < 6)]

            print("Hereee")
            print(events.GenPart.pt)
            
        #if self.partonmatch == 21:
        #    events.GenPart = events.GenPart[(events.GenPart.status == 23) & (abs(events.GenPart.pdgId) == 21)]
        ...
        events.Photon = events.Photon[(((events.Photon.mvaID_WP80 > self.pho_mva80_barrel) & (abs(events.Photon.eta) <= 1.479))
                                      | ((events.Photon.mvaID_WP80 > self.pho_mva80_endcap) & (abs(events.Photon.eta) <= 2.4) 
                                         & (abs(events.Photon.eta) > 1.479))) & (events.Photon.pt > 20)]
        req_pho = (ak.count(events.Photon.pt > 20, axis=1) == 0)
        ...
        _nearParton = selev.FatJet.delta_r(ak.firsts(selev.GenPart)) < 0.8
        #_nearParton = selev.FatJet.delta_r(selev.FatJet.nearest(selev.GenPart)) < 0.8
        kinematic_mask = ((selev.FatJet.pt > 300) & (abs(selev.FatJet.eta) <= 2.4) 
                       & (selev.FatJet.isTight == 1))
        mask_fatjet = _nearParton & kinematic_mask
        ...
        all_fatjet_features = np.stack(
            (
                np.array(ak.flatten(selev.FatJet.pt)),
